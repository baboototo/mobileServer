...
function initEvent(){
	 ...
	 */
	 $("#imgFile").change(function(){
		...
		if($(this).val() != ""){
			ComFileUpload("msgForm", "/android/fileUpload.do", null, function(fileObjs){
				$.each(fileObjs, function(index, item){
					$("#fileSaveNm").val(this.fileSaveNm);
					$("#filePath").val(this.filePath);
					$("#fileOrgNm").val(this.fileOrgNm);
					
 					$("#imgView").show();
					$("#imgView").attr("src", this.imageViewUrl);
				});
			});
		}
     });
	 ...
	 */
	 $("#sendTargetFile").change(function(){
		...
		if($(this).val() != ""){
			ComFileUpload("msgForm", "/android/sendTargetFileUpload.do", null, function(fileObjs){
				$.each(fileObjs, function(index, item){
...
function sendMessage(){
	...
	var sendData = {
			...
			, fileSaveNm: 	$("input[name=fileSaveNm]").val()
			, filePath : 	$("input[name=filePath]").val()
			, fileOrgNm : 	$("input[name=fileOrgNm]").val()
			, sendGB :		$("input:radio[name=sendGB]:checked").val()
			, sendTargetFilePath: $("#sendTargetFilePath").val()
	};
	
	var returnData = ComHttpSync("/android/sendMsg.do", sendData);
...
function goAppBoardList(pageNo){
	var boardData = ComHttpSync("/android/selectAppList.do", {pageNo: pageNo});
	var boardDataHTML = '';
...
function goMessageBoardList(pageNo){
	var messageData = ComHttpSync("/android/selectMessageList.do", {pageNo: pageNo, searchType: "A", searchKeyword: $("#searchKeyword").val()});
	var messageDataHTML = '';
...
function showMessageInfo(messageSeq){
	...
	var resultData = ComHttpSync("/android/selectMessageInfo.do", {seq: messageSeq, type: "A"});
	ComSetBindJsonData(resultData.resultData);
	
	if($("#fileSaveNmInfo").val() != ""){
		$("#imageInfo").show();
		$("#imageInfo").attr("src", resultData.URL+ resultData.resultData.fileFachInfo + "/" + $("#fileSaveNmInfo").val());
	}
}
    ...
    <div class="gnb1">
    	<div class="gnb1_container">
            <ul class="gnb1_list">
                ...
                <li><a href="<c:url value='/android/androidPage.do' />" class="gnb1_item01_select">Android</a></li>
                <li class="gnb1_bg_img"><img src="${context}/resources/images/gnb1_dot.png" alt=""></li>
                ...
                <div class="tree_contents" id="msgPush">
                	<div class="comm_bbswrite">
                		...
                		<form method="post" target="resultFrame" id="msgForm" enctype="multipart/form-data">
                			<input type="hidden" name="type" value="A">				<!-- 메세지전송 Type 설정  -->
							<table cellspacing="0" border="0" summary="입력폼" class="comm_bbswrite_tb">
								...
								<tbody>
									...
									<tr>
										<th>Image</th>
										<td>
											<input type="file" id="imgFile" name="imgFile" style="width: 400px;">
											<input type="hidden" id="fileSaveNm" name="fileSaveNm">
											<input type="hidden" id="filePath" name="filePath">
											<input type="hidden" id="fileOrgNm" name="fileOrgNm">
											
											<div style="width: 100%;;">
												<img id="imgView" src="#" alt="image" style="display: none; margin-top: 10px; margin-bottom: 10px;" />
											</div>
										</td>
									</tr>
									<tr>
						...
						<!-- DATA INFO BOX END -->
						<iframe id="resultFrame" name="resultFrame" width="100%" height="0px" style="left: 0; display:none;"></iframe>
					</div>
						...
						<table cellspacing="0" border="0" summary="입력폼" class="comm_bbswrite_tb">
							...
							<tbody>
								...
								<tr>
									<th>Image</th>
									<td colspan="3">
										<input type="text" id="fileOrgNmInfo" name="fileOrgNmInfo" maxlength="100" style="width: 400px;" disabled="disabled">
										<input type="hidden" id="fileSaveNmInfo" name="fileSaveNmInfo">									
										<div style="width: 100%;">
												<img id="imageInfo" src="#" alt="image"  style="display: none; margin-top: 10px; margin-bottom: 10px;" />
										</div>
									</td>
								</tr>
								<tr>
